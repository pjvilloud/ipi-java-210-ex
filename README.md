...
7. Dans la méthode ```ligneSaisieEstCoherente```, tester le paramètre ```ligneSaisie``` pour vérifier qu'il fait 3 caractères. Si ce n'est pas le cas, afficher le message "Les coordonnées du chiffre et/ou sa valeur doit faire 3 caractères"
8. Dans la méthode ```ligneSaisieEstCoherente```, tester le paramètre ```ligneSaisie``` pour vérifier que le premier caractère est un chiffre entre 0 et 8. En cas d'erreur, afficher le message "L'abscisse et l'ordonnée doivent être compris entre 0 et 8, la valeur entre 1 et 9".
9. Dans la méthode ```ligneSaisieEstCoherente```, tester le paramètre ```ligneSaisie``` pour vérifier que le deuxième caractère est un chiffre entre 0 et 8. En cas d'erreur, afficher le message "L'abscisse et l'ordonnée doivent être compris entre 0 et 8, la valeur entre 1 et 9".
10. Dans la méthode ```ligneSaisieEstCoherente```, tester le paramètre ```ligneSaisie``` pour vérifier que le troisième caractère est un chiffre entre 1 et 9. En cas d'erreur, afficher le message "L'abscisse et l'ordonnée doivent être compris entre 0 et 8, la valeur entre 1 et 9".
